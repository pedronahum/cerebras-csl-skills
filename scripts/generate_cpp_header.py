...
def main() -> int:
    ...
    lines.append("")
    lines.append("// Minimal complete definitions for support types whose real headers")
    lines.append("// the SDK does not ship. Field layouts here are ILLUSTRATIVE — they")
    lines.append("// give std::optional<>, std::vector<>, etc. enough info to parse and")
    lines.append("// instantiate. Real ABI layouts may differ; if you actually link")
    lines.append("// against the SDK .so libraries, replace these with the real headers")
    lines.append("// or ABI-probe each one the way MemcpyOptions was probed.")
    lines.append("template <typename T> struct Point        { T x{}; T y{}; };")
    lines.append("template <typename T> struct AbstractRectangle { Point<T> lo{}; Point<T> hi{}; };")
    lines.append("struct IntVector    { int x{}; int y{}; };          // from das_common")
    lines.append("struct IntRectangle { IntVector lo{}; IntVector hi{}; };  // from das_common")
    ...
    for ename in ("SdkTarget", "MemcpyDataType", "MemcpyOrder"):
        ...
        if values:
            ...
            lines.append("")
    lines.append("// RECOVERED. Field set: pybind splats these 4 kwargs into this struct")
    lines.append("// for every memcpy_*/call invocation. Field order + byte offsets:")
    lines.append("// CONFIRMED by disassembling the pybind wrapper around memcpy_h2d and")
    lines.append("// reading the stack writes that build MemcpyOptions before the call.")
    lines.append("// Evidence in _generated/sdkruntime-memcpyoptions-layout.txt.")
    lines.append("//")
    lines.append("//   mov %al,  0x40(%rsp)  -> offset  0  streaming  (1B + 3B pad)")
    lines.append("//   mov %eax, 0x44(%rsp)  -> offset  4  data_type  (4B)")
    lines.append("//   mov %eax, 0x48(%rsp)  -> offset  8  order      (4B)")
    lines.append("//   mov %al,  0x4c(%rsp)  -> offset 12  nonblock   (1B + 3B pad)")
    lines.append("//   sizeof: 16 bytes.")
    lines.append("struct MemcpyOptions {")
    lines.append("    bool           streaming;    // offset  0 (pybind kwarg #1)")
    lines.append("    MemcpyDataType data_type;    // offset  4 (pybind kwarg #2)")
    lines.append("    MemcpyOrder    order;        // offset  8 (pybind kwarg #3)")
    lines.append("    bool           nonblock;     // offset 12 (pybind kwarg #4)")
    lines.append("};")
    lines.append("static_assert(sizeof(MemcpyOptions) == 16, \"MemcpyOptions layout drift\");")
    lines.append("")
