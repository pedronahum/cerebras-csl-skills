...
| [SKILL-SDKRUNTIME-CPP.md](SKILL-SDKRUNTIME-CPP.md) | The *reconstructed* C++ side of the API. Documents `_generated/cerebras_sdkruntime.hpp` (~340-line reverse-engineered header that parses cleanly under `clang++ -std=c++20 -fsyntax-only`), with a fact-by-fact provenance table (what each piece of info came from + how strong the evidence is). Covers: the `MemcpyOptions` byte layout CONFIRMED by disassembling the pybind wrapper around `memcpy_h2d` and reading stack writes at offsets 0/4/8/12 (guarded by `static_assert(sizeof == 16)`); the four kwargs (`setup_phase_only`, `run_phase_only`, `wio_flows`, `worker`) RESOLVED via runtime assertion strings in `libsdkruntime.so` `.rodata`; the runtime precondition catalog (~80 assertion strings — lifecycle, memcpy regions, layout topology, RPC); the libstdc++ ABI requirement (GLIBCXX_3.4.32 / CXXABI_1.3.14 / GCC 13.x); the `cerebras::SdkRuntime::Impl` PIMPL pattern; the `cerebras::to_words<T>` template helper. The toolchain-reality reasons this header is still documentation rather than a build artifact. |
...
│   └── example_usage.cpp              #   example that parses cleanly under clang++
├── scripts/
