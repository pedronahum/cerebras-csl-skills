---
name: csl
description: Writing and driving CSL (Cerebras Software Language) kernels for the Wafer-Scale Engine. Use for .csl files, the cslc compiler, the Cerebras SDK, run.py host scripts, sdkruntimepybind, or any program targeting WSE-2/WSE-3. CSL side covers syntax, builtins, DSDs/DSRs, tasks, microthreads, fabric routing (colors, RoutingPosition, RAMP/NORTH/SOUTH/EAST/WEST), the cslc/cs_python toolchain, and the bundled standard library. Host side covers SdkRuntime methods (memcpy_h2d / memcpy_d2h / launch / call / send / receive / receive_tofile / task_wait / is_task_done / read_symbol / dump_core / coord_logical_to_physical), the MemcpyOptions kwarg surface (streaming / order / data_type / nonblock), SdkLayout / CodeRegion / ports / streams / compile, the Task / SimfabConfig / SdkExecutionPlatform / SdkCompileArtifacts types, every enum (SdkTarget, MemcpyDataType, MemcpyOrder, Edge, Route, FP16TYPE), the cerebras.sdk.sdk_utils helpers (calculate_cycles, memcpy_view, input_array_to_u32, check_rpc_api), the post-mortem debug pybinds (CSLSymbolTable, SimfabInstructionTrace, SimfabWaveletTrace, RectangleDebugger), and the lower-level memcpy-routing pipeline (routepybind.CslIoRouting, CslWseNetlist / Router / PinTable / Assembler, MEMCPY_XY_ROUTES.elf). Also includes a reconstructed C++ header (_generated/cerebras_sdkruntime.hpp) recovered from demangled .so symbols + pybind11 introspection + pybind .rodata mining + targeted disassembly. The cerebras::MemcpyOptions byte layout (streaming/data_type/order/nonblock at offsets 0/4/8/12; sizeof=16) is CONFIRMED by reading the pybind wrapper's stack writes around memcpy_h2d call sites, guarded by a static_assert in the header. Four previously-undocumented SdkRuntime constructor kwargs (setup_phase_only, run_phase_only, wio_flows, worker) are RESOLVED via runtime assertion strings in libsdkruntime.so .rodata. A runtime precondition catalog (~80 entries: lifecycle order, memcpy region checks, layout topology, RPC validation) is extracted to _generated/sdkruntime-preconditions.txt. libstdc++ ABI requirement: GLIBCXX_3.4.32 / CXXABI_1.3.14 / GCC 13.x. Host-side reference is pinned to SDK 2.10.0 with provenance dumps in _generated/.
---
...
| **Storage classes** (`extern`, `export`, `linkname`, symbol names) | [SKILL-STORAGE.md](SKILL-STORAGE.md) |
...
| **SDK runtime overview** (pinned per-class / per-method reference for the Python + C++ host API; pinned to SDK 2.10.0) | [SKILL-SDKRUNTIME.md](SKILL-SDKRUNTIME.md) |
| **SDK runtime API** (every `SdkRuntime` method, every overload, every kwarg, with C++ cross-references) | [SKILL-SDKRUNTIME-API.md](SKILL-SDKRUNTIME-API.md) |
| **SdkLayout / regions / streams** (`SdkLayout`, `CodeRegion`, ports, routes, `paint`, `compile`) | [SKILL-SDKLAYOUT.md](SKILL-SDKLAYOUT.md) |
...
| **Reconstructed C++ header** (`_generated/cerebras_sdkruntime.hpp` — how it was built, what each piece comes from, where the limits are; mystery kwargs, diagnostic strings, hidden ctors) | [SKILL-SDKRUNTIME-CPP.md](SKILL-SDKRUNTIME-CPP.md) |
